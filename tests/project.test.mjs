...
test('Android renders remote websites in a native preview WebView instead of the restricted iframe', () => {
  ...
  assert.match(activity, /addJavascriptInterface\(new AndroidBridge\(\), "RMSAndroid"\)/);
  ...
  assert.match(activity, /previewWebView = new WebView\(this\)/);
  assert.match(activity, /previewWebView\.setInitialScale\(100\)/);
  assert.match(activity, /setDefaultZoom\(WebSettings\.ZoomDensity\.MEDIUM\)/);
  assert.doesNotMatch(activity, /createConfigurationContext/);
  ...
  assert.match(activity, /onPermissionRequest\(PermissionRequest request\)/);
  assert.match(activity, /"https"\.equalsIgnoreCase\(origin\.getScheme\(\)\)/);
  assert.match(activity, /requestPermissions\(new String\[]\{Manifest\.permission\.CAMERA\}/);
  assert.match(activity, /request\.grant\(new String\[]\{PermissionRequest\.RESOURCE_VIDEO_CAPTURE\}\)/);
  assert.doesNotMatch(activity, /RESOURCE_AUDIO_CAPTURE/);
